...
If `true`, switching the video element into picture-in-picture is disabled. Defautl is `false`.
...
If `true`, the [documentPictureInPicture API](https://developer.chrome.com/docs/web-platform/document-picture-in-picture/) will be used for picture in picture, if available. Currently this is avialable in Chromium 111+. Defaults to `false`, but may default to `true` when the feature has become established.
