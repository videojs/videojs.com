...
> These examples use the obsolete [Flash tech][flash-eol] for illustration of tech ordering with techs which have a greater degree of overlap in sources they can play
...
* Chrome can't play HLS in the standard HTML5 video element on its own, but the [http-streaming][http-streaming] source handler (included in Video.js 7+ by default) _can_ play HLS via [Media Source Extensions][mse] in HTML5. So HLS will be played in the html5 tech.
...
* Safari can play HLS in a standard HTML5 video element, so HLS will be played using the html5 tech.
* Chrome will play HLS in the html5 tech using the [http-streaming][http-streaming] source handler via [Media Source Extensions][mse].
...
Flash is no longer supported as it has reached [end of life][flash-eol].
