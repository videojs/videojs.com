...
* Added new `TitleBar` component, which is not visible in the UI by default _(see below for details)_. As a result, we moved the `BigPlayButton` component to the center of the player by default
...
* Deprecated many old top-level utility methods in favor of utility methods stored on objects _(see below for details)_
